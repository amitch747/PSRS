...
typedef struct {
    ...
    bool *idle_threads; //Used for check function
    bool end; //End of process flag
...
*     job_size - Size of particular job to simplify SJF implementation
* Return:
*     true  - On success
*     false - Otherwise
*/
bool ThreadPool_add_job(ThreadPool_t *tp, thread_func_t func, void *arg, int job_size);
...
/**
* Used within ThreadPool_check to loop through threads and check idle status
* Parameters:
*     tp - Pointer to the ThreadPool which has a "idle_threads" member
* Return:
*     bool - Indicator if all threads are idle or not
*/
bool ThreadPool_idle(ThreadPool_t *tp);